...
def fig2_defensive(base, d, sm, gd):
    ...
    dl = np.stack([as_int_array(dl_raw[j]) for j in range(4)])
    ...
    cm = np.asarray(d["assign_cost"], dtype=np.float64)  # (T, 4, 4)
    for j in range(4):
    ...
    for s in range(T_):
        m = cm[s]
        active_def = [j for j in range(4) if def_alive[j, s]]
        active_att = [
            i for i in range(4)
    ...
    for j in range(4):
        ...
        axNpD.plot(t[alive], npp[alive], color=DEF_COL[j], lw=1.25,
                 label=rf"$n_{{p,D_{{{j}}}}}$")
        axNyD.plot(t[alive], nyy[alive], color=DEF_COL[j], lw=1.25,
                   label=rf"$n_{{y,D_{{{j}}}}}$")
    for ax, lbl, panel in ((axNpD, r"$n_{p,D_j}$ (g)", "(d)"),
        ...
        ax.set_ylabel(lbl)
        ax.set_ylim(-5.6, 5.6)
        ax.legend(loc="upper right", ncol=4, handlelength=1.4, borderaxespad=0.3)
