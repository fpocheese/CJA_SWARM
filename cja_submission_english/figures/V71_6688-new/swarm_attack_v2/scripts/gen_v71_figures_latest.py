...
def _run_episode_steps(obs, record):
    ...
    rec = {
        ...
        'def_an':   [[] for _ in range(n_def)], 'def_lmode':[[] for _ in range(n_def)],
...
def replay_episode(seed, record=True):
    ...
    for step in range(8000):
        ...
        for j, d in enumerate(env0.defensives):
            ...
            an_tot = np.sqrt(d.an_pitch**2 + d.an_yaw**2) / 9.81
            rec['def_an'][j].append(an_tot)
            p = env0.defensive_policies[j]
            rec['def_lmode'][j].append(p.lock_mode)
            rec['def_initial_target'][j].append(p.initial_assigned_target_idx if p.initial_assigned_target_idx is not None else -1)
            rec['def_assigned_target'][j].append(p.assigned_target_idx if p.assigned_target_idx is not None else -1)
            rec['def_ltgt'][j].append(p.current_locked_target_idx if p.current_locked_target_idx is not None else -1)
