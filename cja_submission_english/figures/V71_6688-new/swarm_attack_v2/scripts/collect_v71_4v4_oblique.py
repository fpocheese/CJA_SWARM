#!/usr/bin/env python
"""Collect a successful V71 clone-weight sample for an oblique 4v4 case.
...
DETERMINISTIC_ACTIONS = os.environ.get("V71_DETERMINISTIC", "1").strip().lower() not in {
    "0",
    "false",
    "no",
}
TORCH_SEED_END = int(os.environ.get("V71_TORCH_SEED_END", "40"))
ENV_SEED_COUNT = int(os.environ.get("V71_ENV_SEED_COUNT", "120"))
SKIP_ENV_SEEDS = {
    int(token)
    for token in os.environ.get("V71_SKIP_ENV_SEEDS", "50015,50034,50042,60015,80047").split(",")
    if token.strip()
}
...
def scenario_config(case: str, n_off: int, n_def: int) -> dict:
    if case.strip().lower() == "4v4":
        return dict(FOUR_V4_OBLIQUE_CONFIG)
    return {"n_offensive": n_off, "n_defensive": n_def}


def make_raw_env(n_off: int, n_def: int, seed: int | None = None, case: str = ""):
    os.environ.setdefault("FOV_REWARD_PROFILE", "v69teamsurvive")
    raw_env = FOVPenetrationEnv(
        config=scenario_config(case, n_off, n_def),
        scenario="scenario_1",
    )
...
def init_worker(case: str):
    ...
    _RAW_ENV, _ENV = make_raw_env(n_off, n_def, case=case)
...
def parse_trial(trial):
    if isinstance(trial, (tuple, list)):
        return int(trial[0]), int(trial[1])
    seed = int(trial)
    torch_seed = None if DETERMINISTIC_ACTIONS else seed
    return seed, torch_seed


def run_scan_one(trial):
    env = _ENV
    policies = _POLICIES
    if env is None or policies is None:
        raise RuntimeError("Worker not initialized")

    seed, torch_seed = parse_trial(trial)
    ...
    return {
        "case": _CASE,
        "seed": int(seed),
        "torch_seed": int(torch_seed) if torch_seed is not None else "",
...
def record_success(case: str, seed: int, out_dir: Path, torch_seed: int | None = None):
    n_off, n_def = parse_case(case)
    if torch_seed is not None:
        torch.manual_seed(torch_seed)
    raw_env, env = make_raw_env(n_off, n_def, seed=seed, case=case)
    ...
    summary = {
        "case": case,
        "seed": int(seed),
        "torch_seed": int(torch_seed) if torch_seed is not None else None,
        "deterministic_actions": bool(DETERMINISTIC_ACTIONS),
        "model_dir": str(MODEL_DIR),
        "clone_map": {str(i): int(i % 4) for i in range(n_off)},
        "n_offensive": n_off,
        "n_defensive": n_def,
        "scenario_variant": "seeded_4v4_default_geometry" if case.strip().lower() == "4v4" else "clone_default",
        "scenario_overrides": scenario_config(case, n_off, n_def),
...
def build_trials(case: str, seed_base: int, max_eps: int):
    if DETERMINISTIC_ACTIONS:
        return [seed_base + i for i in range(max_eps)]

    explicit_torch = os.environ.get("V71_TORCH_SEEDS", "").strip()
    if explicit_torch:
        torch_seeds = [int(token) for token in explicit_torch.split(",") if token.strip()]
    else:
        torch_seeds = [42] + [seed for seed in range(TORCH_SEED_END) if seed != 42]
    explicit_env = os.environ.get("V71_ENV_SEEDS", "").strip()
    if explicit_env:
        env_candidates = [int(token) for token in explicit_env.split(",") if token.strip()]
    else:
        env_candidates = [seed_base + i for i in range(ENV_SEED_COUNT)]
    env_seeds = [seed for seed in env_candidates if seed not in SKIP_ENV_SEEDS]
    trials = [(env_seed, torch_seed) for torch_seed in torch_seeds for env_seed in env_seeds]
    return trials[:max_eps]


def run_case(case: str, seed_base: int, scan_eps: int, max_eps: int, workers: int, out_root: Path):
    trials = build_trials(case, seed_base, max_eps)
    print(
        f"=== scanning {case}: seed_base={seed_base}, scan_eps={scan_eps}, "
        f"max_trials={len(trials)} deterministic={DETERMINISTIC_ACTIONS} ===",
        flush=True,
    )
    all_rows = []
    batch_start = 0
    while batch_start < len(trials):
        batch_end = min(batch_start + scan_eps, len(trials))
        batch = trials[batch_start:batch_end]
        with mp.Pool(processes=max(1, min(workers, len(batch))), initializer=init_worker, initargs=(case,)) as pool:
            for row in pool.imap_unordered(run_scan_one, batch):
                all_rows.append(row)
                print(
                    f"{case} ep={len(all_rows):03d} seed={row['seed']} torch_seed={row['torch_seed']} success={int(row['success'])} "
        ...
        if batch_start < len(trials):
            print(f"{case}: no success yet, extending scan to {min(batch_start + scan_eps, len(trials))} trials", flush=True)
    ...
    best_torch_seed = int(best["torch_seed"]) if str(best.get("torch_seed", "")).strip() else None
    print(
        f"{case}: recording best success seed={best['seed']} torch_seed={best.get('torch_seed', '')} "
        f"best={best['best_min_dist_m']:.2f}m",
        flush=True,
    )
    summary = record_success(case, int(best["seed"]), case_dir, torch_seed=best_torch_seed)
...
def main():
    ...
    parser.add_argument("--seed-base-4v4", type=int, default=int(os.environ.get("V71_4V4_SEED_BASE", "94000")))
