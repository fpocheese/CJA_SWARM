#!/bin/bash
# V63: strict phase split + terminal PN action-consistency reward.
# Hypothesis: terminal obs exposes only HVT LOS rates, so the actor needs a
# reward that directly connects those LOS rates to pitch/yaw actions, not only
# range attraction. No action-space, physics, hit-threshold, or scenario changes.
set -e
