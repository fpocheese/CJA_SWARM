#!/bin/bash
# V58B: Aggressive stage-gated reward profile (stronger near-end constraints).
# Hypothesis: enforce terminal geometry harder (near strike + lateral miss penalty)
# to push min_d from O(100m) toward hit radius while keeping run stable.
...
export FOV_REWARD_PROFILE=v58b
export CUDA_VISIBLE_DEVICES=0
export OMP_NUM_THREADS=2
export MKL_NUM_THREADS=2

echo "[V58B] start=$(date) model_dir=$V45_MODEL_DIR profile=$FOV_REWARD_PROFILE"
conda run --no-capture-output -n rlgpu python -u scripts/train_fov_penetration_mappo.py \
  --algorithm_name mappo \
  --experiment_name v58B_stage_gated_aggr \
  --scenario scenario_1 \
  --ap_config v28 \
  --cuda \
