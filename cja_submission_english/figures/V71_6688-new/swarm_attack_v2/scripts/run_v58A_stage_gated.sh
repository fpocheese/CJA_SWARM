#!/bin/bash
# V58A: Stage-gated reward + primary-attacker credit shaping.
# Hypothesis: fix "close but not hit" by reducing near-range approach white-leech,
# strengthening near heading/closing constraints, and assigning stronger progress
# credit to a single primary attacker while keeping support agents as decoys.
...
export FOV_REWARD_PROFILE=v58a
export CUDA_VISIBLE_DEVICES=0
export OMP_NUM_THREADS=2
export MKL_NUM_THREADS=2

echo "[V58A] start=$(date) model_dir=$V45_MODEL_DIR profile=$FOV_REWARD_PROFILE"
conda run --no-capture-output -n rlgpu python -u scripts/train_fov_penetration_mappo.py \
  --algorithm_name mappo \
  --experiment_name v58A_stage_gated \
  --scenario scenario_1 \
  --ap_config v28 \
  --cuda \
