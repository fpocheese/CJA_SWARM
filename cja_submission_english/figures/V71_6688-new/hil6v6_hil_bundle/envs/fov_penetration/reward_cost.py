...
def compute_terminal_rewards(offensives, hvt, config, ap_data=None):
    ...
    # ============================================================
    # V69 shared team bonus: at episode end, if >=1 attacker hit
    # HVT or is still alive, give a big positive shared bonus.
    # User mandate: any attacker reaching terminal phase = team win.
    # ============================================================
    team_survival_bonus = rc.get('team_survival_bonus', 0.0)
    if team_survival_bonus > 0.0 and (N_hit >= 1 or N_alive >= 1):
        terminal_r += team_survival_bonus
