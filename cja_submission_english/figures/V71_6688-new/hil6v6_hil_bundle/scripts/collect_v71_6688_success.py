#!/usr/bin/env python
"""Collect successful V71 clone-weight samples for fixed 6v6 and 8v8 cases.
...
def parse_case(case: str) -> tuple[int, int]:
    ...
    if token == "8v8":
        return 8, 8
    raise ValueError(f"Unsupported case {case!r}; expected 6v6 or 8v8")
...
def main():
    ...
    parser.add_argument("--cases", nargs="+", default=["6v6", "8v8"])
    ...
    out_root = Path(args.out_root) / f"{stamp}_v71_6688_clone_success"
    out_root.mkdir(parents=True, exist_ok=True)
    started = time.time()
    summaries = {}
    for case in args.cases:
        seed_base = args.seed_base_6v6 if case == "6v6" else args.seed_base_8v8
