...
def run_case(case: str, seed_base: int, scan_eps: int, max_eps: int, workers: int, out_root: Path):
    target_successes = int(os.environ.get("V71_4V4_TARGET_SUCCESSES", "5"))
    print(
        f"=== scanning {case}: seed_base={seed_base}, scan_eps={scan_eps}, "
        f"max_eps={max_eps}, target_successes={target_successes} ===",
        flush=True,
    )
    all_rows = []
    summaries = []
    recorded_seeds = set()
    batch_start = 0
    while batch_start < max_eps and len(summaries) < target_successes:
        ...
        new_successes = [
            row for row in sorted(all_rows, key=lambda row: row["seed"])
            if row["success"] and row["seed"] not in recorded_seeds
        ]
        for row in new_successes:
            if len(summaries) >= target_successes:
                break
            case_name = f"case_{len(summaries) + 1:02d}_seed{int(row['seed'])}"
            case_dir = out_root / case_name
            print(
                f"{case}: recording success {len(summaries) + 1}/{target_successes} "
                f"seed={row['seed']} best={row['best_min_dist_m']:.2f}m -> {case_dir}",
                flush=True,
            )
            summary = record_success(case, int(row["seed"]), case_dir)
            summary["case_name"] = case_name
            summary["scan_success_count_so_far"] = sum(1 for item in all_rows if item["success"])
            summary["scan_n_episodes_so_far"] = len(all_rows)
            with (case_dir / "summary.json").open("w") as f:
                json.dump(summary, f, indent=2, ensure_ascii=False)
            write_scan_csv(case_dir / "scan_episodes.csv", all_rows)
            summaries.append(summary)
            recorded_seeds.add(row["seed"])

        batch_start = batch_end
        if batch_start < max_eps and len(summaries) < target_successes:
            print(
                f"{case}: successes={len(summaries)}/{target_successes}, "
                f"extending scan to {min(batch_start + scan_eps, max_eps)} episodes",
                flush=True,
            )

    write_scan_csv(out_root / f"{case}_scan_episodes.csv", all_rows)
    top_summary = {
        "case": case,
        "seed_base": seed_base,
        ...
        "success_seeds": [int(item["seed"]) for item in summaries],
        "case_dirs": [item.get("case_name", "") for item in summaries],
    }
    with (out_root / f"{case}_scan_summary.json").open("w") as f:
        json.dump(top_summary, f, indent=2, ensure_ascii=False)
    if len(summaries) < target_successes:
        print(f"{case}: only found {len(summaries)} successes after {len(all_rows)} episodes", flush=True)
    return summaries
...
def main():
    ...
    out_root = Path(args.out_root) / f"{stamp}_v71_4v4_multi_success"
